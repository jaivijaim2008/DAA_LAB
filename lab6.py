...
def print_dp_table(m, n):
    """Print the DP cost table in formatted manner"""
    print('\nDP Cost Table m[i][j]:')
    print(f'{"":>6}', end='')
...
def matrix_chain_naive(dims):
    ...
    Used for comparison - much slower for large chains
    Time: O(2^n)
    """
    n = len(dims) - 1
...
def count_operations(parens_str):
    """Count the number of matrix multiplications from parenthesization string"""
    return parens_str.count('x')
