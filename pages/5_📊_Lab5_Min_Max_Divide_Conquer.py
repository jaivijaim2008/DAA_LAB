...
def visualize_dc_tree(arr, low, high, depth=0, parent=None):
    """
    Generate tree structure for visualization
    Returns list of nodes with their info
    """
    nodes = []
    mid = (low + high) // 2
    ...
    if low == high:
        nodes.append({
            ...
            'parent': parent,
            'range': f"[{low}:{high}]",
            'result': f"min={arr[low]}, max={arr[low]}",
            'comparisons': 0
        })
    elif high == low + 1:
        nodes.append({
            'id': f"{low}-{high}",
            'label': f"[{arr[low]},{arr[high]}]",
            'depth': depth,
            'parent': parent,
            'range': f"[{low}:{high}]",
            'result': f"min={min(arr[low],arr[high])}, max={max(arr[low],arr[high])}",
            'comparisons': 1
        })
    else:
        nodes.append({
        ...
        nodes.extend(visualize_dc_tree(arr, low, mid, depth + 1, f"{low}-{high}"))
        nodes.extend(visualize_dc_tree(arr, mid + 1, high, depth + 1, f"{low}-{high}"))
